...
import os
import load_dotenv
load_dotenv.load_dotenv("/Users/bartek/Documents/ai_persp/accenture-hackathon/src/.env")
...
class EmbeddingGenerator:
    ...
    def create_embedding(
        self, text_to_embed: str, model: str = "embeds-hackathon"
    ) -> List[float]:
        embed: List[float] = (
...
class CSVLoader:
    ...
    def vector_store_creation(
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            # Dispatch phase
            result=[]
            futures = [executor.submit(self.process_row, row, embedding_field_name, exclude_fields) 
